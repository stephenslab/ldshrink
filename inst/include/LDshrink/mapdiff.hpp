...
class mapdiff_functor {
  ...
  const Scalar cutoff;
public:
  mapdiff_functor(const Eigen::Map<const ArgType> arg, const Scalar m_,const Scalar Ne_,const Scalar cutoff_) :
    m_vec(arg),
    m(m_),
    Ne(Ne_),
    cutoff(cutoff_) {
...
makeMapDiff(const Eigen::Map<const ArgType>& arg,const Scalar m,const Scalar Ne,const Scalar cutoff)
{
  ...
  return MatrixType::NullaryExpr(arg.size(), arg.size(), mapdiff_functor<ArgType>(arg.derived(),m,Ne,cutoff));
}
