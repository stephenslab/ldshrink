...
LDshrinkWriter::LDshrinkWriter(const std::vector<int> &inp,
    ...
    : snpnames_a(names), snpnames_b(names), p_a(inp.size()), p_b(p_a),
      write_annotations(write_annotations) {
  data_store.reserve(((p_a * p_a - p_a) / 2 + p_a) / 5);
  if (write_annotations) {
    anno_data_store.reserve(((p_a * p_a - p_a) / 2 + p_a) / 5);
  }
}
...
LDshrinkWriter::LDshrinkWriter(const std::pair<std::vector<T>,std::vector<T>> &inp,
			       ...
			       const bool write_annotations):
			       snpnames_a(names.first),
				 snpnames_b(names.second),
				 p_a(inp.first.size()),
				 p_b(inp.second.size()),
				 write_annotations(write_annotations)
			       {
...
template<typename T>
bool operator<(const MatTup<T> &a, const MatTup<T> &b){
  return(a.idx()<b.idx());
}
...
void LDshrinkWriter::finalize(){
  if(write_annotations){
    ...
    if (!std::equal(anno_data_store.begin(), anno_data_store.end(),
                    data_store.begin(),
                    [](const MatTup<T> &a, const MatTup<T> &b) {
	...
	if((ra.row()!=rb.row()) || (ra.col() !=rb.col())){
	  ...
	  Rcpp::Rcerr<<"data_store:" <<ra.row()<<","<<ra.col()<<","<<ra.value()<<std::endl;
	  Rcpp::Rcerr<<"anno_data_store:" <<rb.row()<<","<<rb.col()<<","<<rb.value()<<std::endl;
	  Rcpp::stop("unequal elements in	anno_store and anno_data_store");
	}
      }
 ...
 Rcpp::DataFrame LDshrinkWriter::toAnnotationDataFrame(const bool use_rownames)const {
   ...
   return (DataFrame::create(_["rowsnp"] = rowid, _["colsnp"] = colid,
    ...
    for (size_t i = 0; i < num_trip; i++) {
      auto trip = data_store[i];
      auto atrip = anno_data_store[i];
      rowid(i) = snpnames_a(trip.row());
      colid(i) = snpnames_b(trip.col());
      rvec(i) = trip.value();
      anno(i) = atrip.value();
    }
    return (DataFrame::create(_["rowsnp"] = rowid, _["colsnp"] = colid,
...
Rcpp::S4 LDshrinkWriter::dsCMatrix() const {
  ...
  ans.slot("Dimnames") = List::create(snpnames_a, snpnames_b);
  return (ans);
}
