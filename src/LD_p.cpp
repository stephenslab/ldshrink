...
//[[Rcpp::export]]
SEXP sparse_ldshrink(Eigen::MatrixXd data, std::vector<double> mapd,
                     Rcpp::IntegerVector indices, const double m,
                     const double Ne, const double cutoff, const int total_size,
                     const bool progress = true,
                     const bool useldshrink = true) {

  const size_t p = mapd.size();
  std::vector<size_t> tindices(p);
  std::copy(indices.begin(), indices.end(), tindices.begin());
  ldshrink_data<double, Eigen::ColMajor> dat(std::move(data), std::move(mapd),
                                             std::move(tindices));
  dat.calc_vars();
...
//[[Rcpp::export]]
SEXP sparse_ldshrink_p(Eigen::MatrixXd data_a, Eigen::MatrixXd data_b,
                       std::vector<double> mapd_a, std::vector<double> mapd_b,
                       Rcpp::IntegerVector indices_a,
                       Rcpp::IntegerVector indices_b, const double m,
                       const double Ne, const double cutoff,
                       const int total_size, const bool progress = false,
                       const bool useldshrink = true) {

  const size_t p_a = mapd_a.size();
  std::vector<size_t> tindices_a(p_a);
  std::copy(indices_a.begin(), indices_a.end(), tindices_a.begin());
  ldshrink_data<double, Eigen::ColMajor> dat_a(
      std::move(data_a), std::move(mapd_a), std::move(tindices_a));
  dat_a.calc_vars();
  const size_t p_b = mapd_b.size();
  std::vector<size_t> tindices_b(p_b);
  std::copy(indices_b.begin(), indices_b.end(), tindices_b.begin());
  ldshrink_data<double, Eigen::ColMajor> dat_b(
      std::move(data_b), std::move(mapd_b), std::move(tindices_b));
  dat_b.calc_vars();
  Sparse_cov<double, Eigen::ColMajor> sp(m, Ne, cutoff, 0, progress);
  if (useldshrink) {
...
//[[Rcpp::export]]
Rcpp::DataFrame ld2df(Eigen::MatrixXd data, std::vector<double> mapd,
                      Rcpp::RObject rsid, const double m, const double Ne,
                      const double cutoff, const double r2cutoff = 0.01,
...
//[[Rcpp::export]]
Rcpp::DataFrame ld2df_p(Eigen::MatrixXd data_a, Eigen::MatrixXd data_b,
                        std::vector<double> mapd_a, std::vector<double> mapd_b,
                        Rcpp::RObject rsid_a, Rcpp::RObject rsid_b,
                        const double m, const double Ne, const double cutoff,
  ...
  if ((rsid_a.sexp_type() != INTSXP) && (rsid_a.sexp_type() != STRSXP)) {
    Rcpp::stop("rsid vectors must be integer or	string vectors (of the same "
               "type)");
  }
  ...
  ldshrink_data<double, Eigen::ColMajor> dat_a(
      std::move(data_a), std::move(mapd_a), std::move(indices_a));
  dat_a.calc_vars();
  const size_t p_b = mapd_b.size();
  std::vector<size_t> indices_b(p_b);
  std::iota(indices_b.begin(), indices_b.end(), 0);
  ldshrink_data<double, Eigen::ColMajor> dat_b(
      std::move(data_b), std::move(mapd_b), std::move(indices_b));
  dat_b.calc_vars();
  Sparse_cov<double, Eigen::ColMajor> sp(m, Ne, cutoff, r2cutoff, progress);
  if (useldshrink) {
    sp.ldshrink(&dat_a, &dat_b);
