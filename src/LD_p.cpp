...
//[[Rcpp::export]]
Rcpp::DataFrame ld2df_p(const Eigen::Map<Eigen::MatrixXd> scaled_data, const Eigen::ArrayXd mapd,Rcpp::StringVector rsid,const double m, const double Ne, const double cutoff,const double r2cutoff=0.01){
   using namespace tbb;
  ...
  //    for(size_t ir=0; ir<p_od; ir++){
	...
	tripletList.push_back(Trip(i,j,cov));
      ...
      // else{
      // 	if(i!=j)
      // 	Rcpp::Rcerr<<"i:"<<i<<" j:"<<j<<" r2:"<<cov<<std::endl;
      // }
    }
  }
					       });
